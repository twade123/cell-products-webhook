...
if os.environ.get('GHL_API_KEY'):
    ...
    try:
        ...
        print("Note: GHL_COMPANY_ID is optional until obtained from GHL support")
        sys.exit(1)
...
def create_subaccount_from_survey_data(payload):
    ...
    try:
        # Extract business name from payload (form fields are at top level, not in survey_data)
        ...
        # Debug logging for business name extraction
        logging.info(f"🔍 Business name extracted: '{business_name}'")
        
        # Extract contact fields from payload (form fields are at top level)
        ...
        # Note: companyId will be required once obtained from GHL support
        company_id = CONFIG.get('company_id')
        if company_id:
            logging.info(f"✅ Using Company ID: {company_id}")
        else:
            logging.warning("⚠️ Company ID not set - sub-account creation may fail until obtained from GHL support")
        
        # Clean phone number
        clean_phone = phone.replace('+1', '').replace('-', '').replace('(', '').replace(')', '').replace(' ', '')
        
        # Build sub-account data according to GHL v1 API requirements (flat structure)
        subaccount_data = {
            "businessName": business_name,  # Business name for v1 API
        ...
        # Add companyId only if available (waiting for GHL support to provide)
        if company_id:
            subaccount_data["companyId"] = company_id
...
def handle_survey_completion():
    ...
    try:
        ...
        # Business name validation will be done in create_subaccount_from_survey_data function
        # (Removed redundant extraction here)
        ...
        # Create sub-account using full payload (form fields are at top level)
